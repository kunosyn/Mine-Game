...
TerrainManager::TerrainManager(sf::RenderWindow* window, const sf::Texture& blockAtlas)
	: m_window(window), m_seed(0), m_blockAtlas(blockAtlas)
{
	selectSeed();
}
...
void TerrainManager::selectSeed (void) {
	...
	std::cout << "[TerrainManager]: Seed: " << m_seed << std::endl;
	std::cout << "[TerrainManager]: Frequency: " << TERRAIN_FREQUENCY << std::endl;
	std::cout << "[TerrainManager]: Height Addition: " << HEIGHT_ADDITION << std::endl;
	std::cout << "[TerrainManager]: Height Multiplier: " << HEIGHT_MULTIPLIER << std::endl << std::endl;
}
...
void TerrainManager::renderTerrain (const sf::FloatRect viewBounds) const {
	for (const Chunk& chunk : m_terrain) {
		m_window->draw(chunk.getVertices(), &m_blockAtlas);
	}
}
